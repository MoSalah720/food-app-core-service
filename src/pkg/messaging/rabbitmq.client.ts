...
import { IMessageBroker } from "./message_broker.interface";
import amqp from "amqp-connection-manager";

export interface RabbitMQConfig{
    url: string;
    reconnectionInitialMs?: number; 
}
...
export class RabbitMQClient implements IMessageBroker{
    ...
    async connect(): Promise<void> {
        ...
        this.connection= amqp.connect([this.config.url],{
            reconnectTimeInSeconds:Math.max(1,this.config.reconnectionInitialMs??500/1000)
        });
